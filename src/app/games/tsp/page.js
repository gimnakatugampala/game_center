...
class TSPGame extends Component {
  ...
  returnHome = () => {
    ...
    const visitedCities = new Set(
      playerRouteArray.filter((c) => c !== homeCity)
    );
  ...
  isRouteComplete = () => {
    ...
    return (
      selectedCities.every((city) => uniqueVisited.has(city)) &&
      visitedCities.length === selectedCities.length
    );
  };

  saveToDatabaseCall = async (
    playerName,
    homeCity,
    selectedCities,
    playerRoute,
    ...
    while (route.length < n) {
      ...
      for (let i = 0; i < n; i++) {
        if ((visited & (1 << i)) === 0) {
          ...
          const dist =
            distances[cities[current]][cities[i]] +
            (memo.get(key) || tsp(newVisited, i));
  ...
  submitAnswer = async () => {
    try {
      ...
      if (!playerAnswer.trim()) {
        this.setState({
          error: "Please enter the shortest distance",
          isLoading: false,
        });
        return;
      }
      ...
      if (playerRouteArray.length === 0) {
        this.setState({
          error: "Please build your route using the route builder",
          isLoading: false,
        });
        return;
      }
      ...
      const nnResult = this.nearestNeighborAlgorithm(
        distances,
        citiesToVisit,
        homeCity
      );
      ...
      results.push({
        ...
        type: "Iterative",
        description:
          "Greedy heuristic approach - always picks nearest unvisited city",
      });
      ...
      results.push({
        ...
        type: "Recursive",
        description:
          "Exhaustive search - tries all possible permutations recursively",
      });

      const dp_start = performance.now();
      const dpResult = this.dynamicProgrammingTSP(
        distances,
        citiesToVisit,
        homeCity
      );
      ...
      results.push({
        ...
        type: "Iterative",
        description:
          "Optimal solution using memoization - stores subproblem results",
      });

      const optimalDistance = Math.min(...results.map((r) => r.distance));
      const tolerance = 1;
      const isCorrect =
        Math.abs(playerGuessedDistance - optimalDistance) <= tolerance;
          ...
          <div className="flex justify-center items-center min-h-[70vh] px-5 py-10">
            <div className="bg-slate-800/90 backdrop-blur-xl rounded-[30px] p-12 max-w-2xl w-full shadow-[0_30px_80px_rgba(0,0,0,0.6)] border-2 border-indigo-500/30 text-center animate-fadeInUp">
              ...
              <p className="text-lg text-slate-300 leading-relaxed mb-10">
                Challenge yourself to find the shortest path through multiple
                cities. Test your problem-solving skills against three powerful
                algorithms!
              </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_450px] gap-8 max-w-[1400px] mx-auto">
            ...
            <div className="lg:col-span-2 bg-slate-800/80 backdrop-blur-md px-8 py-4 rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.3)] border border-indigo-500/20">
              <span className="text-lg text-slate-300">
                👤 Playing as:{" "}
                <strong className="text-white bg-gradient-to-r from-indigo-400 to-purple-600 bg-clip-text text-transparent text-xl font-extrabold">
                  {playerName}
                </strong>
              </span>
            </div>
            ...
            <div className="bg-slate-900/60 backdrop-blur-md rounded-3xl p-5 shadow-[0_20px_60px_rgba(0,0,0,0.5)] border border-white/10">
              <div className="relative w-full flex justify-center items-center">
                <InteractiveMap
                  ...
                  showAllConnections={
                    gameState === "select_cities" || gameState === "playing"
                  }
                />
              </div>
            </div>
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-5 gap-2.5 mb-5">
                      {CITIES.map((city) => (
                        ...
                        >
                          ...
                          {city === homeCity && (
                            <span className="block text-[0.7rem] mt-1 opacity-90">
                              Home
                            </span>
                          )}
                        </button>
                ...
                <div className="bg-slate-800/80 backdrop-blur-md rounded-3xl p-8 shadow-[0_20px_60px_rgba(0,0,0,0.5)] border border-white/10 animate-slideInRight">
                  ...
                  <div className="bg-gradient-to-br from-indigo-500/20 to-purple-600/20 rounded-xl p-5 mb-6 border-2 border-indigo-500/30 text-center">
                    ...
                    <p className="m-0 text-slate-300 leading-relaxed text-sm">
                      <strong>Step 1:</strong> Build your route visiting all
                      cities
                      <br />
                      <strong>Step 2:</strong> Calculate and enter the shortest
                      possible distance
                      <br />
                      <strong>Step 3:</strong> Submit and see if you found the
                      optimal solution!
                    </p>
                  </div>
                  ...
                  <div className="my-6 p-5 bg-black/30 rounded-xl border border-blue-500/20">
                    ...
                    <div className="flex flex-col gap-4">
                      <div className="grid grid-cols-1 md:grid-cols-[auto_1fr_auto] gap-2.5 items-center">
                        <label
                          htmlFor="citySelect"
                          className="text-slate-300 font-semibold text-sm whitespace-nowrap"
                        >
                          Select next city:
                        </label>
                        <select
                        ...
                        >
                          ...
                          {this.getAvailableCities().map((city) => (
                            <option
                              key={city}
                              value={city}
                              className="bg-slate-900 text-white p-2.5"
                            >
                      ...
                      <button
                        ...
                        disabled={
                          playerRouteArray.length === 0 ||
                          playerRouteArray[playerRouteArray.length - 1] ===
                            homeCity
                        }
                      >
                        🏠 Return Home
                    ...
                    <div className="mt-4 text-center">
                      {this.isRouteComplete() ? (
                        <div className="p-4 bg-green-500/20 border-2 border-green-500/50 rounded-xl text-green-300 font-bold text-lg animate-pulseSuccess">
                          ✅ Route Complete! Your route distance:{" "}
                          {this.calculateRouteDistance()} km
                        </div>
                      ) : (
                  ...
                  <div className="bg-black/30 rounded-xl p-5 my-5 border-2 border-yellow-400/30">
                    ...
                    <p className="text-slate-300 text-sm leading-relaxed mb-5 text-center px-4 py-4 bg-yellow-400/10 rounded-lg">
                      ...
                      than your current route? Think about different orders and
                    ...
                    <div className="mb-5">
                      ...
                      <small className="block mt-1 text-sm text-slate-500 italic">
                        💡 Hint: Your current route is{" "}
                        {this.isRouteComplete()
                          ? this.calculateRouteDistance()
                          : "?"}{" "}
                        km. Can you do better?
                      </small>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="w-full px-6 py-4 border-none rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 text-white text-xl font-bold cursor-pointer transition-all duration-300 shadow-[0_10px_30px_rgba(102,126,234,0.4)] relative overflow-hidden hover:translate-y-[-2px] hover:shadow-[0_15px_40px_rgba(102,126,234,0.6)] active:translate-y-0 disabled:opacity-60 disabled:cursor-not-allowed disabled:shadow-none"
                    disabled={
                      isLoading || !this.isRouteComplete() || !playerAnswer
                    }
                  >
                  ...
                  <div className="bg-slate-900/50 rounded-2xl p-6 mb-6 border border-slate-700/30">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
                      ...
                      <div className="bg-slate-800/60 rounded-xl p-5 text-center border-2 border-slate-700/40 hover:border-blue-500/50 hover:-translate-y-1 transition-all duration-300">
                        ...
                        <div className="text-sm text-slate-400 leading-relaxed min-h-[40px]">
                          {Math.abs(
                            this.state.playerGuessedDistance -
                      ...
                      <div className="bg-gradient-to-br from-emerald-900/30 to-green-900/30 rounded-xl p-5 text-center border-2 border-emerald-500/50 hover:border-emerald-400 hover:-translate-y-1 transition-all duration-300">
                        ...
                        <div className="text-sm text-emerald-200/70 leading-relaxed min-h-[40px]">
                          {this.state.playerActualDistance ===
                          this.state.optimalDistance
                            ? "✅ You found it!"
                            : `${(
                                this.state.playerActualDistance -
                    ...
                    <div className="text-center">
                      {result === "win" ? (
                        <div className="bg-emerald-500/20 border-2 border-emerald-500/40 text-emerald-100 p-4 rounded-xl leading-relaxed">
                          ...
                          You correctly identified the shortest distance!
                          {this.state.playerActualDistance ===
                            this.state.optimalDistance && (
                            <span>
                              {" "}
                              And you found the optimal route on your first try!
                            </span>
                          )}
                        </div>
                      ) : (
                        <div className="bg-blue-500/20 border-2 border-blue-500/40 text-blue-100 p-4 rounded-xl leading-relaxed">
                          ...
                          Compare your route with the optimal solutions below to
                          see how different algorithms approach this problem.
                        </div>
                      )}
                    </div>
                  </div>
                    ...
                    <>
                      <div className="bg-slate-900/50 rounded-2xl p-6 mb-6 border border-slate-700/30">
                        ...
                        <div className="bg-slate-800/60 rounded-xl p-4 mb-4">
                          <strong className="block mb-3 text-slate-300">
                            Route:
                          </strong>
                        ...
                        <div className="inline-flex items-center px-4 py-2 rounded-full bg-gradient-to-r from-violet-500/30 to-purple-500/30 border border-violet-500/50 text-white font-semibold text-sm">
                          {currentRoute.type === "Recursive"
                            ? "🔄 Recursive Implementation"
                            : "➡️ Iterative Implementation"}
                        </div>
                      </div>
                      ...
                      <table className="w-full text-sm border-collapse">
                        <thead>
                          <tr className="bg-violet-900/40">
                            <th className="p-3 text-left text-xs uppercase tracking-wider text-slate-300 font-semibold">
                              Algorithm
                            </th>
                            <th className="p-3 text-left text-xs uppercase tracking-wider text-slate-300 font-semibold">
                              Type
                          ...
                          {algorithmResults.map((algo, index) => (
                            <tr
                              ...
                              className={`cursor-pointer transition-all border-b border-slate-800/50 hover:bg-violet-900/20 ${
                                algo.distance ===
                                Math.min(
                                  ...algorithmResults.map((a) => a.distance)
                                )
                    ...
                    <div className="mt-5 bg-slate-800/40 rounded-xl p-5 border-l-4 border-violet-500">
                      ...
                      <ul className="space-y-3 text-slate-300 text-sm leading-relaxed">
                        <li className="border-b border-slate-700/50 pb-3">
                          <strong className="text-white bg-gradient-to-r from-pink-400 to-rose-400 bg-clip-text text-transparent">
                            Brute Force (Recursive):
                          </strong>{" "}
                          Guarantees optimal solution but slowest - Time grows
                          factorially: 5 cities = 120 permutations, 10 cities =
                          3.6M permutations!
                        </li>
                        <li className="border-b border-slate-700/50 pb-3">
                          <strong className="text-white bg-gradient-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent">
                            Dynamic Programming (Iterative):
                          </strong>{" "}
                          Also optimal but much faster using memoization -
                        ...
                        <li className="border-b border-slate-700/50 pb-3">
                          ...
                          Fastest but approximate - Good for quick estimates but
                          may miss optimal route by 10-25%
                        </li>
                        <li>
                          <strong className="text-white bg-gradient-to-r from-violet-400 to-purple-400 bg-clip-text text-transparent">
                            Recursive vs Iterative:
                          </strong>{" "}
                          Brute Force uses recursion (elegant, natural for
                          permutations), while DP and NN use iteration (explicit
                          control, better for large inputs)
                        </li>
                      </ul>
                    </div>
                  </div>
